...
W=array(rep(0,2*4*n),dim=c(2,4,n))
for(i in 1:length(data[,1])){
  W[1,1,i]=1
  W[1,2,i]=0
  W[1,3,i]=Z[i]
...
Ds<-matrix(rep(0,4*4),4,4)
diag(Ds) <- 1

Dr<-matrix(rep(0,4*4),4,4)
Dr[!(diag(Dr))] <- 0.3
diag(Dr) <- 1
...
eta=rmnorm(ntrial, c(0,0,0,0), Ds%*%Dr%*%Ds)

# Store values 
holdbet<-matrix(rep(0,8*SIM),SIM,8)
holdeta<-array(rep(0,ntrial*4*SIM),dim=c(ntrial,4,SIM))
holdD<-array(rep(0,4*4*SIM),dim=c(4,4,SIM))
holdsig<-array(rep(0,2*2*SIM),dim=c(2,2,SIM))

vd <- 5 # 5 for scenario 1, 9 for scenario 2 
vsigma <- 3

F<-matrix(rep(0,4*4),4,4)
diag(F) <- 1/6 # 1/(q+2) where q = length of random effects (4 in scenario 1, 8 in scenario 2)
...
while(sim<=SIM){
  ...
  et=array(0,c(2,1))
  for(i in 1:n){
    mu[1,]=beta[1]+beta[3]*Z[i]+beta[5]*X[i]+beta[7]*Z[i]*X[i]
    mu[2,]=beta[2]+beta[4]*Z[i]+beta[6]*X[i]+beta[8]*Z[i]*X[i]
    for(k in 1:ntrial){
      if(trialID[i]==k){
        et[1,]=eta[k,1]+eta[k,3]*Z[i]
        et[2,]=eta[k,2]+eta[k,4]*Z[i]
      }}
  ...
  x <- array(rep(0,4*4*ntrial),dim=c(4,4,ntrial))
  y <- array(rep(0,4*ntrial),dim=c(4,ntrial))
  
  # S2 inv 
  for(i in 1:n){
    for(k in 1:ntrial){
      if(trialID[i]==k){
        x1=t(W[,,i])%*%solve(S)%*%W[,,i]
        x[,,k]=x[,,k]+x1
      }}}
  ...
  for(i in 1:n){
    for(k in 1:ntrial){
      if(trialID[i]==k){
        y1=t(W[,,i])%*%solve(S)%*%(Y[,i]-X[,,i]%*%t(t(beta)))
        y[,k]=y[,k]+y1
      }}}
...
for (i in 1:SIM){
  ...
  daa.0=D[1,1]+ss*((1/n1.0)+(1/n0.0))
  daa.1=D[1,1]+ss*((1/n1.1)+(1/n0.1))
  
  keep.psiSP13[i,1] <- 1 - pmvnorm(lower = -Inf, upper = 0, mean = holdbet[i,3], sigma = D[1,1]) -
    pmvnorm(lower = -Inf, upper = 0, mean = holdbet[i,4], sigma = D[2,2]) +
    2*pmvnorm(lower = -Inf, upper = c(0,0), mean = holdbet[i,3:4], sigma = D)
  keep.psiSP13[i, 2] <- 1 - pmvnorm(lower = -Inf, upper = 0, mean = holdbet[i,3] + holdbet[i,7], sigma = D[1,1]) -
    pmvnorm(lower = -Inf, upper = 0, mean = holdbet[i,4] + holdbet[i,8], sigma = D[2,2]) +
    2*pmvnorm(lower = -Inf, upper = c(0,0), mean = holdbet[i,3:4] + holdbet[i,7:8] , sigma = D)
  keep.psiSP123[i, 1] <- 1 - pmvnorm(lower = -Inf, upper = 0, mean = holdbet[i,4], sigma = D[2,2]) +
    pmvnorm(lower = -Inf, upper = c(0,0), mean = holdbet[i, 3:4], sigma = D)
  keep.psiSP123[i, 2] <- 1 - pmvnorm(lower = -Inf, upper = 0, mean = holdbet[i,4] + holdbet[i,8], sigma = D[2,2]) +
    pmvnorm(lower = -Inf, upper = c(0,0), mean = holdbet[i,3:4] + holdbet[i,7:8] , sigma = D)
  
  
  keep.s[i,1]=(holdbet[i,3])-(daa.0/D[1,2])*((holdbet[i,4])+qnorm(0.95)*sqrt(D[2,2]-(D[1,2]^2/daa.0)))
  keep.s[i,2]=(holdbet[i,3] + holdbet[i,7])-(daa.1/D[1,2])*((holdbet[i,4] + holdbet[i,8])+qnorm(0.95)*sqrt(D[2,2]-(D[1,2]^2/daa.1)))
